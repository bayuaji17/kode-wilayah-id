...
class RegionService {
  ...
  private loadData() {
    const csv = readFileSync('base.csv', 'utf-8')
    ...
    for (const line of lines) {
      ...
      if (parts.length === 1) {
        ...
        this.regenciesByProvince.get(province_code)!.push({ code, name, province_code })
        ...
        this.districtsByRegency.get(regency_code)!.push({ code, name, regency_code })
        ...
        this.villagesByDistrict.get(district_code)!.push({ code, name, district_code })
      }
    }
    
    console.log(`Loaded: ${this.provinces.size} provinces, ${this.regencies.size} regencies, ${this.districts.size} districts, ${this.villages.size} villages`)
  }
  ...
  getRegencies(provinceId: string) {
    if (!this.provinces.has(provinceId)) {
      return status(404, { success: false as const, error: 'Province not found' })
    }
  ...
  getDistricts(regencyId: string) {
    ...
    if (!data) {
      return status(404, { success: false as const, error: 'Regency not found' })
    }
  ...
  getVillages(districtId: string) {
    ...
    if (!data) {
      return status(404, { success: false as const, error: 'District not found' })
    }
